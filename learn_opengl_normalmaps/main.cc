//
// Created by abdoe on 22/06/2025.
//
...
int main() {
  ...
  // -------------------------
  Shader shader("_main/learn_opengl_normalmaps/shaders/shader.vert",
                "_main/learn_opengl_normalmaps/shaders/shader.frag");

  // load textures
  // -------------
  unsigned int diffuseMap =
      loadTexture("_main/learn_opengl_normalmaps/textures/brickwall.jpg");
  unsigned int normalMap = loadTexture(
      "_main/learn_opengl_normalmaps/textures/brickwall_normal.jpg");
...
unsigned int loadTexture(char const *path) {
  ...
  std::string local_path = LocalPaths::getLocalPath(path);

  int width, height, nrComponents;
  unsigned char *data =
      stbi_load(local_path.c_str(), &width, &height, &nrComponents, 0);
  if (data) {
    GLenum format;
    if (nrComponents == 1)
      format = GL_RED;
